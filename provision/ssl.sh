...
# Add permissions to certificates and keys
chmod 664 -R *.key
chmod 664 -R *.crt

# Modify nginx-configmap-dh.yaml
sed 's/^/    /' dhparam.pem > temp.crt
sed -i -E -e '/^ *-+BEGIN /,/^ *-+END / {/-END /r temp.crt' -e ';d;}' ../../nginx/nginx-configmap-dh.yaml

# Modify nginx-configmap-crt.yaml
sed 's/^/    /' api.crt > temp.crt
sed -i -E -e '/^ *-+BEGIN /,/^ *-+END / {/-END /r temp.crt' -e ';d;}' ../../nginx/nginx-configmap-crt.yaml

# Modify nginx-configmap-key.yaml
sed 's/^/    /' api.key > temp.crt
sed -i -E -e '/^ *-+BEGIN /,/^ *-+END / {/-END /r temp.crt' -e ';d;}' ../../nginx/nginx-configmap-key.yaml
